 ...
 */
 ...
 static int i2c_write_byte(struct adxl345_dev *dev, uint8_t reg, uint8_t data)
 {
    ...
    return (ret == ESP_OK) ? ERR_OK : ERR_I2C_WRITE;                                            
 }
 ...
 static int i2c_read_byte(struct adxl345_dev *dev, uint8_t reg, uint8_t *data)
{
   ...
   return (ret == ESP_OK) ? ERR_OK : ERR_I2C_READ;
}
                                 ...
                                 enum adxl345_data_rate rate)
{
   ...
   if (!dev) {
      LOG_ERROR("Failed to allocate memory for ADXL345 device");
      return NULL;
   }
   ...
   if (ret != ERR_OK || dev_id != 0xE5) {
      LOG_ERROR("Failed to read device ID or device ID mismatch (0x%02X)", dev_id);
      free(dev);
      return NULL;
   }
   LOG_INFO("ADXL345 device ID: 0x%02X", dev_id);
   ...
   LOG_INFO("ADXL345 initialized successfully (I2C addr: 0x%02X, range: %dG, rate: %dHz)", dev->i2c_addr, range, rate);
   return dev;
}
...
void adxl345_deinit(struct adxl345_dev *dev)
{
   if (dev) {
      ...
      i2c_write_byte(dev, ADXL345_POWER_CTL, power_ctl);
      free(dev);
      LOG_INFO("ADXL345 device deinitialized");
   }
}

int adaxl345_read_raw(struct adxl345_dev *dev, int16_t *x, int16_t *y, int16_t *z)
{
   if (!dev || !x || !y || !z) return ERR_INVALID_PARAM;

   uint8_t buf[6];
   int ret = i2c_read_bytes(dev, ADXL345_DATAX0, buf, sizeof(buf));
   if (ret != ERR_OK) {
      LOG_ERROR("Failed to read any raw data");
      return ret;
   }
...
{
   if (!dev || !x_g || !y_g || !z_g) return ERR_INVALID_PARAM;

   int16_t x_raw, y_raw, z_raw;
   int ret = adxl345_read_raw(dev, &x_raw, &y_raw, &z_raw);
   if (ret != ERR_OK) {
      LOG_ERROR("Failed to read raw data needed for g conversion");
      return ret;
   }
...
int adxl345_set_test(struct adxl345_dev *dev)
{
   /* 简易自检: 读取静止时 Z 轴应接近 1g */
   float x, y, z;
   int ret = adxl345_read_g(dev, &x, &y, &z);
   if (ret != ERR_OK) {
      LOG_ERROR("Self-test failed: unable to read data");
      return ret;
   }

   if (fabsf(z - 1.0f) > 0.5f) {
      LOG_ERROR("Self-test failed: Z axis reading out of expected range (%.2fg)", z);
      return ERR_SENSOR_NOT_READY;
   }
   LOG_INFO("Self-test passed: Z axis reading = %.2fg", z);
   return ERR_OK;
}

int adxl345_set_range(struct adxl345_dev *dev, enum adxl345_range range)
{
   if (!dev) return ERR_INVALID_PARAM;
   dev->range = range;
   update_scale_factor(dev);
   ...
   if (ret != ERR_OK) {
      LOG_ERROR("Failed to read data format register for range update");
      return ret;
   }
