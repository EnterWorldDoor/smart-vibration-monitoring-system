...
### 时间同步
- 所有时间操作必须通过 `time_sync.h` 中的函数进行。
- 时间同步函数：`time_sync.h` 中定义的函数，用于同步系统时间。
...
### 系统监控
- 如果需要用系统监视功能，必须要用 `system_monitor.h` 中的函数。
- 系统监视函数：`system_monitor.h` 中定义的函数，用于监控系统运行状态。

### 单元测试
- 所有模块必须有单元测试。
- 单元测试必须在相应模块文件夹目录下生成模块名_test.c 文件，文件名与模块名相同。
- 单元测试必须使用 `unity.h` 进行测试。

### 远程固态升级
- 如果需要远程固态升级，必须在 `ota_update.h` 中定义升级接口。
- 升级接口：`ota_update.h` 中定义的函数，用于升级固件。
- 升级接口必须在 `ota_update.c` 中实现。
- 如果需要调用远程固态升级接口的，则使用项目提供的 `ota_update.h` 中的函数。`

### 其他
- 如果用到环形缓冲，必须在 `common/ringbuf.h` 中定义。
- 环形缓冲：`common/ringbuf.h` 中定义的函数，用于实现环形缓冲。`
- 环形缓冲必须在 `common/ringbuf.c` 中实现。
- 环形缓冲模块仍存在不足，需要及时修复，以符合企业级要求。
