...
// Cache is a thread-safe in-memory deduplication cache with TTL.
type Cache struct {
	mu    sync.Mutex
...
// MakeKey builds a dedup key from source site, device, and alert source.
func MakeKey(sourceSite, sourceDevice, alertSource string) string {
	return fmt.Sprintf("%s|%s|%s", sourceSite, sourceDevice, alertSource)
}
