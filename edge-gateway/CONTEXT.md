...
**知识点**: `alloc_candev()` / `register_candev()` / `struct net_device_ops` / `alloc_can_skb()` / `can_rx_offload_*` (NAPI)

### D2: IIO Vibration Device

**业务关联**: 振动数据在 TimescaleDB 中，无法通过标准 Linux 传感器接口读取。

**设计**: 注册 IIO 虚拟设备，sysfs 暴露 `/sys/bus/iio/devices/iio:device0/in_rms_x_raw` 等。daemon 从 DB 读最新值后更新。

**知识点**: `struct iio_dev` / `iio_device_register()` / `struct iio_chan_spec` / IIO sysfs 接口

### D3: RAM Buffer Block Device (blk-mq)

**业务关联**: data-aggregator 每 2s 大量小写入 TimescaleDB，磨损 eMMC。

**设计**: 注册 `/dev/edgevib-buffer` 块设备，内存环形缓冲。TimescaleDB WAL 挂载在此设备上，定期 flush 到 eMMC。

**知识点**: `struct gendisk` / `blk_mq_tag_set` + `blk_mq_ops.queue_rq` / `blk_mq_start_request()` / `kmap()`/`kunmap()`

### D4: Software RTC Driver

**业务关联**: Orange Pi 无电池 RTC，断电时钟归零→TimescaleDB 分区键乱序。

**设计**: 注册 `rtc-edgevib` 平台设备。开机从 `/var/lib/edgevib/last_time` 读上次关机时间设初始时钟，关机时写回。

**知识点**: `struct rtc_device` / `struct rtc_class_ops` / `devm_rtc_device_register()` / `struct platform_driver` / `module_platform_driver()`

### D5: HWMON Motor Health

**业务关联**: `sensors` 命令是 Linux 运维标配。Grafana hwmon 插件可直接使用。

**设计**: 注册 hwmon 设备，sysfs 暴露 `temp1_input`（电机温度）/ `curr1_input`（电机电流）/ `fan1_input`（振动RMS）。

**知识点**: `struct hwmon_device` / `hwmon_device_register_with_info()` / `HWMON_CHANNEL_INFO()`
